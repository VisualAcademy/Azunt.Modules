...
{
    public class ProgressiveTypesTableBuilder
    {
        private readonly string _masterConnectionString;
        private readonly ILogger<ProgressiveTypesTableBuilder> _logger;

        public ProgressiveTypesTableBuilder(string masterConnectionString, ILogger<ProgressiveTypesTableBuilder> logger)
        {
        ...
        public void BuildTenantDatabases()
        {
            ...
            foreach (var connStr in tenantConnectionStrings)
            {
                try
                {
                    EnsureProgressiveTypesTable(connStr);
                    _logger.LogInformation($"ProgressiveTypes table processed (tenant DB): {connStr}");
                }
                catch (Exception ex)
                {
        ...
        public void BuildMasterDatabase()
        {
            try
            {
                EnsureProgressiveTypesTable(_masterConnectionString);
                _logger.LogInformation("ProgressiveTypes table processed (master DB)");
            }
            catch (Exception ex)
            {
        ...
        private void EnsureProgressiveTypesTable(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                // Check if 'ProgressiveTypes' table exists
                var cmdCheck = new SqlCommand(@"
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                    WHERE TABLE_NAME = 'ProgressiveTypes'", connection);

                int tableCount = (int)cmdCheck.ExecuteScalar();

                if (tableCount == 0)
                {
                    // Create 'ProgressiveTypes' table if it doesn't exist
                    var cmdCreate = new SqlCommand(@"
                        CREATE TABLE [dbo].[ProgressiveTypes] (
                    ...
                    _logger.LogInformation("ProgressiveTypes table created.");
                }
                ...
                foreach (var (columnName, columnDefinition) in expectedColumns)
                {
                    var cmdColCheck = new SqlCommand(@"
                        ...
                        WHERE TABLE_NAME = 'ProgressiveTypes' AND COLUMN_NAME = @ColumnName", connection);
                    ...
                    if (columnExists == 0)
                    {
                        var cmdAlter = new SqlCommand($@"
                            ALTER TABLE [dbo].[ProgressiveTypes] 
                            ADD [{columnName}] {columnDefinition}", connection);
                        cmdAlter.ExecuteNonQuery();

                        _logger.LogInformation($"Column added to ProgressiveTypes: {columnName} ({columnDefinition})");
                    }
                }

                // Insert default rows if the table is empty
                var cmdCountRows = new SqlCommand("SELECT COUNT(*) FROM [dbo].[ProgressiveTypes]", connection);
                int rowCount = (int)cmdCountRows.ExecuteScalar();

                if (rowCount == 0)
                {
                    var cmdInsertDefaults = new SqlCommand(@"
                        INSERT INTO [dbo].[ProgressiveTypes] (Active, IsDeleted, Created, CreatedBy, Name, DisplayOrder)
                        VALUES
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial ProgressiveType 1', 1),
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial ProgressiveType 2', 2)
                        ", connection);

                    int inserted = cmdInsertDefaults.ExecuteNonQuery();
                    _logger.LogInformation($"ProgressiveTypes default data inserted: {inserted} rows.");
                }
            }
        }
        ...
        public static void Run(IServiceProvider services, bool forMaster, string? optionalConnectionString = null)
        {
            try
            {
                var logger = services.GetRequiredService<ILogger<ProgressiveTypesTableBuilder>>();
                ...
                var builder = new ProgressiveTypesTableBuilder(connectionString, logger);
            ...
            {
                var fallbackLogger = services.GetService<ILogger<ProgressiveTypesTableBuilder>>();
                fallbackLogger?.LogError(ex, "Error while processing ProgressiveTypes table.");
            }
        }
    }
}