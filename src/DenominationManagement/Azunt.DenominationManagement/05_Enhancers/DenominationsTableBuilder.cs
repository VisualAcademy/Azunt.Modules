...
{
    public class DenominationsTableBuilder
    {
        private readonly string _masterConnectionString;
        private readonly ILogger<DenominationsTableBuilder> _logger;

        public DenominationsTableBuilder(string masterConnectionString, ILogger<DenominationsTableBuilder> logger)
        {
        ...
        public void BuildTenantDatabases()
        {
            ...
            foreach (var connStr in tenantConnectionStrings)
            {
                try
                {
                    EnsureDenominationsTable(connStr);
                    _logger.LogInformation($"Denominations table processed (tenant DB): {connStr}");
                }
                catch (Exception ex)
                {
        ...
        public void BuildMasterDatabase()
        {
            try
            {
                EnsureDenominationsTable(_masterConnectionString);
                _logger.LogInformation("Denominations table processed (master DB)");
            }
            catch (Exception ex)
            {
        ...
        private void EnsureDenominationsTable(string connectionString)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();

                // Check if 'Denominations' table exists
                var cmdCheck = new SqlCommand(@"
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
                    WHERE TABLE_NAME = 'Denominations'", connection);

                int tableCount = (int)cmdCheck.ExecuteScalar();

                if (tableCount == 0)
                {
                    // Create 'Denominations' table if it doesn't exist
                    ...
                    _logger.LogInformation("Denominations table created.");
                }
                ...
                foreach (var (columnName, columnDefinition) in expectedColumns)
                {
                    var cmdColCheck = new SqlCommand(@"
                        ...
                        WHERE TABLE_NAME = 'Denominations' AND COLUMN_NAME = @ColumnName", connection);
                    ...
                    if (columnExists == 0)
                    {
                        var cmdAlter = new SqlCommand($@"
                            ALTER TABLE [dbo].[Denominations] 
                            ADD [{columnName}] {columnDefinition}", connection);
                        cmdAlter.ExecuteNonQuery();

                        _logger.LogInformation($"Column added to Denominations: {columnName} ({columnDefinition})");
                    }
                }

                // Insert default rows if the table is empty
                var cmdCountRows = new SqlCommand("SELECT COUNT(*) FROM [dbo].[Denominations]", connection);
                int rowCount = (int)cmdCountRows.ExecuteScalar();

                if (rowCount == 0)
                {
                    var cmdInsertDefaults = new SqlCommand(@"
                        INSERT INTO [dbo].[Denominations] (Active, IsDeleted, Created, CreatedBy, Name)
                        VALUES
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial Denomination 1'),
                            (1, 0, SYSDATETIMEOFFSET(), 'System', 'Initial Denomination 2')", connection);

                    int inserted = cmdInsertDefaults.ExecuteNonQuery();
                    _logger.LogInformation($"Denominations default data inserted: {inserted} rows.");
                }
            }
        }
        ...
        public static void Run(IServiceProvider services, bool forMaster)
        {
            try
            {
                var logger = services.GetRequiredService<ILogger<DenominationsTableBuilder>>();
                ...
                var builder = new DenominationsTableBuilder(masterConnectionString, logger);
            ...
            {
                var fallbackLogger = services.GetService<ILogger<DenominationsTableBuilder>>();
                fallbackLogger?.LogError(ex, "Error while processing Denominations table.");
            }
        }
    }
}
